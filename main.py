...
client.chat_postMessage(channel='#test_vrijeme', text="Živ sam!")


def send_tomorrow_forecast(channel):
    weather_response = metaweather.get_forecast().json()
    block_builder = BlockBuilder(CityForecast(weather_response), channel)  # BLOCK
...
# method for getting daily weather report
...
def hour_change():
    ...
    if text == '':
        ...
        client.chat_postMessage(channel=channel_id,
                                text='*Slanje obavijesti o prognozi biti će svaki dan u ' + text + 'h*')
